...
3. **Spaced Repetition Engine (Autoset):** Once a topic is completed, the system can automatically clone it ("autoset") as a new "Revision" topic. The `notBefore` date would be automatically set to `Date.now() + 3 days` (and progressively longer for subsequent revisions) to build a proper, automated spaced repetition queue.
