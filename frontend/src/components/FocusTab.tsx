                ...
                >
                  ...
                  <>
                    <div className="col-span-1 flex items-center space-x-2">
                      <div className="w-2 h-8 flex flex-col justify-center space-y-[2px] opacity-20 group-hover:opacity-100 transition-opacity mr-2">
              ...
              {localTomorrowPlan.map((topic: any, i: number) => {
                return (
                  <Reorder.Item
                    ...
                    exit={{ opacity: 0, scale: 0.95 }}
                    transition={{ duration: 0.4, delay: i * 0.05 }}
                    className="grid grid-cols-12 gap-4 items-center px-4 py-3 border border-border bg-bg/50 hover:bg-bg hover:border-mutedFg group cursor-grab active:cursor-grabbing transition-all"
                  >
                    <div className="col-span-1 flex items-center space-x-2">
                      <div className="w-2 h-8 flex flex-col justify-center space-y-[2px] opacity-20 group-hover:opacity-100 transition-opacity mr-2">
                        <div className="w-1 h-1 bg-fg rounded-full"></div>
                    ...
                    <div className="col-span-1 text-center font-mono font-bold text-md opacity-50" style={{ color: exams?.find(e => e.id === topic.examId)?.color || 'var(--color-accent)' }}>
                      {topic.size}
                    </div>
                    <div className="col-span-1 flex justify-center">
                    </div>
                    <div className="col-span-2 text-right flex items-center justify-end font-mono text-xs text-mutedFg">
                      <Clock className="w-3 h-3 mr-2" />
                      {Math.round(topic.scheduledMinutes || topic.expectedDurationMinutes || 60)}m
                    </div>
                  </Reorder.Item>
